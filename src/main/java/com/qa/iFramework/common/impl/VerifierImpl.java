...
import com.qa.iFramework.common.xml.Entity.ResponseParameter;
...
public class VerifierImpl implements IVerifier {
    ...
    public Result VerifyResult(Step result, List<ResponseParameter> responseParams, Map<String, String> entry, List<Object> preExecutedResults, CommandEntity commandEntity) throws Exception{
        ...
        if(!StringUtils.isEmptyOrSpace(result.getUrl())){
            ...
            executeResult.setEnvironment("UNKNOWN");
        }
        ...
        try {
            ...
            switch (result.getResult().getClass().getName()) {
                ...
                case "java.lang.Boolean":
                    resultObject.put(Constants.bool, Boolean.valueOf(String.valueOf(result.getResult())));
                    break;
                    ...
                    try {
                        if (!StringUtils.isEmptyOrSpace(result.getType()) && "jar".equalsIgnoreCase(result.getType().trim())) {
                            if (JsonUtil.isJsonString(resultString)) {
                                JSONObject actualJarResultObject = JSONObject.parseObject(resultString);
                                resultObject.put(Constants.jar, actualJarResultObject);
                            } else{
                                resultObject.put(Constants.jar, resultObject);
                            }
                        } else {
                            if (JsonUtil.isJsonString(resultString)) {
...
//                        MyAssert.assertNotNull(comparingJSONObject, "expect result is " + expectCompare);
                    ...
                    switch (expectCompare.getAction().toLowerCase()) {
                        case "equal":
                            ...
                            } else if (expectCompare.getType().toLowerCase().equals("boolean")){
                                MyAssert.assertTrue(Boolean.valueOf(expectCompare.getValue()),
                                        "boolean type is not equal in Verifying");
                            }
